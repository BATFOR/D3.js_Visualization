...
~~ function renderGraph() {
    ...
    simulation = d3.forceSimulation() //启动力学仿真    //force之后返回的依然是forceSimulation对象
        .force("charge", d3.forceManyBody()) //添加力学仿真模型
        .force("links", d3.forceLink())
        .force("center", d3.forceCenter(800 / 2, 600 / 2))
        .force("collision", d3.forceCollide(30))
        .nodes(nodes); //设定节点数组        //将json格式转化为力学图可用的格式

    //下面力学模型的参数配置也可以在初始化时就设置好,如向心力center的初始化
    simulation.force("links").links(links).distance(150); //设置弹簧力模型参数，连线数组和连线长度
    simulation.force("charge").strength(-200); //顶点的电荷数。该参数决定是排斥还是吸引，数值越小越互相排斥
    ...
    circle.append("svg:title").text(function(node) {
        switch (node.label) {
            case 'Device':
                return node.factoryCode;
            case 'Application':
                return node.applicationName;
            case 'Position':
                return node.positionCode;
            case 'Manufacturer':
                return node.manufacturerName;
            case 'IotInfrastructure':
                return node.iotInfrastructureName;
            case 'DeviceAdmin':
                return node.name;
        }
    });
    ...
    function showUnlock(node, i) {
        unlockPath = d3.select('.g_circle_' + i).append("path").attr('class', 'g_circle_path')
            ...
            .on("click", function(d) { unlockNodeFixed(d, this); })
        ...
        d3.selectAll(".unlock_a").on("click", (d) => { unlockNodeFixed(d, this); })
